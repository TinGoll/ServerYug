...
const at_order_service_1 = __importDefault(require("../services/at-order-service"));
const p_queue_1 = __importDefault(require("p-queue"));
const queue = new p_queue_1.default({ concurrency: 1 });
// /api/at-order/
const router = (0, express_1.Router)();
// /api/at-order/data
router.get('/data', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const query = atOrder_1.default.get('get_barcodes');
        const barcodes = yield dataBase_1.default.executeRequest(query);
...
router.get('/journal-names', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const query = atOrder_1.default.get('get_barcodes');
        let barcodes = yield dataBase_1.default.executeRequest(query);
        return res.status(200).json({ barcodes });
    }
    catch (e) {
        next(e);
...
router.get('/salary-transactions/:id', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // Проверка прав
        if (!idJournalName)
            throw ApiError_1.default.BadRequest('Не корректный ID журнала.');
        ...
        // Конец проверки прав.
        const transactions = yield dataBase_1.default.executeRequest(`
                select T.ID,
        ...
        if (!transactions || transactions.length == 0)
            throw ApiError_1.default.BadRequest('Список пуст.');
...
router.get('/salary-report/:idtransaction', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c;
    ...
    try {
        ...
        const salary = yield dataBase_1.default.executeRequest(`
        ...
        for (const sectorName of sectorsName) {
            ...
            const sectorID = (_a = salary.find(s => s.SECTOR == sectorName)) === null || _a === void 0 ? void 0 : _a.ID_SECTOR;
            const sector = { id: sectorID, name: sectorName, otherTransactoins: {}, orders: [] };
            const otherTrans = yield dataBase_1.default.executeRequest(`select T.NAME, T.DESCRIPTION, T.AMOUNT, T.MODIFER, T.TRANS_COMMENT
            ...
            for (const id of ordersId) {
                const idJournal = (_b = salary.find(s => s.ID == id)) === null || _b === void 0 ? void 0 : _b.ID_JOIRNAL;
                const itmOrderNum = (_c = salary.find(s => s.ID == id)) === null || _c === void 0 ? void 0 : _c.ITM_ORDERNUM;
...
router.get('/preliminary-calculation/:id', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // Проверка прав
        if (!idJournalName)
            throw new Error('Не корректный id журнала.');
        const journals = yield virtualJournalsFun_1.default.permissionSet(user);
        const journal = journals.find(j => j.id == idJournalName); // получаем обект журнала.
        if (!journal)
            throw ApiError_1.default.Forbidden(['У тебя нет прав на получение данных этого журнала. Обратись а администатору.']);
        ...
        const query = `select O.ID, O.ITM_ORDERNUM, J.ID as ID_JOIRNAL, P.ID_SECTOR, S.NAME as SECTOR, 
                            ...
                            cast(coalesce(O.ORDER_FASADSQ, 0) * W.PRICE as decimal(8,2)) as MONEY, P.OPTIONAL
                                ...
                                where ${(yield user.permissionCompare(permissions[0])) ? '' : '(P.OPTIONAL != -1) and'}
                                E.ID is null and j.id_journal_names in (${journal.j.join(', ')})`;
        const isCanEditAllWorks = yield user.permissionCompare(permissions[1]);
        const salary = yield dataBase_1.default.executeRequest(query);
        const sectors = [];
        if (salary.length == 0)
            return res.status(500).json({ errors: ['В текущем периоде, по этому журналу нет принятых заказов.'], message: 'Список пуст' });
        const sectorsName = [...new Set(salary.map(s => s.SECTOR))].filter(s => {
            return s != null || s != undefined;
        });
        for (const sectorName of sectorsName) {
            ...
            const users = yield dataBase_1.default.executeRequest(`select E.NAME from EMPLOYERS E where E.ID_SECTOR = ${sectorID}`);
            sector.otherTransactoins = {
...
router.patch('/close-billing-period', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const permissions = [
    ...
    const user = yield (0, users_1.getUserToToken)(req.get('Authorization'));
    // Конец проверки токена.
    if (!(yield user.permissionCompare(permissions[0])))
        throw ApiError_1.default.Forbidden(['У тебя нет прав на закрытие расчетного периода. Обратись к администатору.']);
    // Проверка, есть ли заказы для закрытия периода.
    const { sectors } = req.body;
    let isEmpty = true;
    if (sectors && sectors.length)
        for (const sector of sectors)
            if (sector && (sector === null || sector === void 0 ? void 0 : sector.orders.length))
                isEmpty = false;
    if (isEmpty)
        throw ApiError_1.default.BadRequest('Список пуст');
    // Конец проверки.
    try {
        const transaction = yield dataBase_1.default.executeRequest(`
                                ...
                                values (
                                    ${user.id}, 
                                    '${(0, date_format_parse_1.format)(new Date(), 'DD.MM.YYYY HH:mm:ss')}', 
                                    '${(0, date_format_parse_1.format)(new Date(), 'DD.MM.YYYY')}',
        ...
        for (const sector of sectors) {
            for (const order of sector.orders) {
                ...
                for (const work of order.works) {
                    if (work.isDeleted) {
                        yield dataBase_1.default.executeRequest(`delete from cost_of_work w where w.id = ${work.workOfCostId}`);
                        continue;
                    }
                    else if (work.workOfCostId) {
                        if (work.isEdited) {
                            yield dataBase_1.default.executeRequest(`update COST_OF_WORK W set W.PRICE = ${work.price} where W.ID = ${work.workOfCostId}`);
                        }
                    }
                    else {
                        yield dataBase_1.default.executeRequest(`insert into COST_OF_WORK (ID_JOURNAL, ID_WORK_PRICE, PRICE) values (${order.idJournal}, ${work.workId}, ${work.price})`);
                    }
                }
            }
            yield dataBase_1.default.executeRequest(`
            ...
            if (sector.otherTransactoins.data.length > 0) {
                for (const otherTransaction of sector.otherTransactoins.data) {
                    ...
                    if (!otherTransaction.amount)
                        continue;
                    if (otherTransaction.modifer !== 1 && otherTransaction.modifer !== -1)
                        continue;
                    yield dataBase_1.default.executeRequest(`
        ...
        const [trSum] = yield dataBase_1.default.executeRequest(`
            ...
            `);
        const cashlow = yield dataBase_1.default.executeRequest(`
...
// /api/at-order/add
function queueTransferOrders(data) {
    return __awaiter(this, void 0, void 0, function* () {
        return queue.add(() => at_order_service_1.default.transferOrders(data));
    });
}
router.post('/add', [
    (0, express_validator_1.check)('idTransfer', 'Передающий участок не может быть пустым.').exists(),
    (0, express_validator_1.check)('idAccepted', 'Принимающий участок не может быть пустым.').exists()
], (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const data = req.body;
        const result = yield queueTransferOrders(data);
