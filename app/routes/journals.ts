...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const sectors = _.uniqWith(
        result
          .filter((s) => {
            if (s.SECTOR?.toUpperCase() == "Колеровка".toUpperCase())
              return false;
            if (s.SECTOR?.toUpperCase() == "Шлифовка Станок".toUpperCase())
              return false;
            if (s.SECTOR?.toUpperCase() == "ЧПУ".toUpperCase())
              return false;
            if (
              s.SECTOR?.toUpperCase() ==
              "Склад упакованных заказов".toUpperCase()
            )
              return false;
            if (s.SECTOR?.toUpperCase() == "Упаковка профиля".toUpperCase())
              return false;
            return true;
          })
          .map((s) => {
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (journals.length == 0)
        throw ApiError.BadRequest(defaultError, ["Список журналов пуст."]);
    ...
    try {
      ...
      if (comment.dataId) {
        if (!comment.text || comment.text == "") {
          ...
          const isDeleted = await system.deleteCommentToId(comment.dataId);
          return res.status(200).json({ id: isDeleted });
        }
        // Если не пустое, обновляем коммент.
        const editComment = await system.editComment(comment);
        if (editComment) {
          return res.status(201).json({ dataId: editComment.id });
        }
      }
      // Если ID коммента 0 - добавляем новый.
      const newComment = await system.addCommentToOrder(
        comment.orderId,
        comment
      );
      ...
      return res.status(201).json({ dataId: newComment?.id });
    } catch (e) {
      next(e);
    }
  }
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const dateFirst: Date | undefined = d1
        ? convertToDate(d1, "dd/mm/yyyy")
        : undefined;
      const dateSecond: Date | undefined = d2
        ? convertToDate(d2, "dd/mm/yyyy")
        : undefined;
      ...
      if (!journal)
        throw ApiError.Forbidden([
          "У тебя нет прав на получение данных этого журнала. Обратись а администатору.",
        ]);
...
router.get("/:id", async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!allowed)
      throw ApiError.Forbidden([
        "У тебя нет прав на получение данного журнала. Обратись а администатору.",
      ]);
    // Проверка прав завершена.
    let journal: JournalSectorDto[] = [];
    if (!id)
      throw ApiError.BadRequest(defaultError, [
        "Некорректный идентификатор журнала.",
      ]);
    ...
    for (const sectorId of sectorsId) {
      ...
      sector.overdue = sectorOrders
        .filter((o) => o.datePlan?.valueOf()! < toDay)
        .map((o) => {
          const order: JournalOrderDto = {
            id: o.id,
            itmOrderNum: o.itmOrderNum,
...
const convertToDate = (date: string, format: string): Date => {
  ...
  var normalizedFormat: string = format
    .toLowerCase()
    .replace(/[^a-zA-Z0-9]/g, "-");
