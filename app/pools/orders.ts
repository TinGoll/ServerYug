...
const getAllOrders = async (
  ...
  try {
    ...
    const isGetOrdersAllAllowed = await user.permissionCompare(
      "Orders [orders] get orders all"
    );
    if (!isGetOrdersAllAllowed)
      throw ApiError.Forbidden([
        "Не хватает прав, на получение данных журнала заказов.",
      ]);
    ...
    const result = await db.executeRequest(
      ordersQuery.get("get_orders", options) as string
    );

    // Права пользователей.

    const isViewCity = await user.permissionCompare(
      "Orders [orders] get field City"
    );
    const isViewCost = await user.permissionCompare(
      "Orders [orders] get field Cost"
    );
    const isViewTotalCost = await user.permissionCompare(
      "Orders [orders] get field Cost"
    );
    const isViewPay = await user.permissionCompare(
      "Orders [orders] get field Pay"
    );
    const isViewDebt = await user.permissionCompare(
      "Orders [orders] get field Debt"
    );

    const isViewDateFirstStage = await user.permissionCompare(
      "Orders [orders] get field DateFirstStage"
    );
    const isViewDateSave = await user.permissionCompare(
      "Orders [orders] get field DateSave"
    );
    const isViewDatePlanPack = await user.permissionCompare(
      "Orders [orders] get field DatePlanPack"
    );
    ...
    const orders: OrderHeader[] = result.map((o) => {
      ...
      if (isViewDebt) order.debt = Math.abs(o.ORDER_DEBT);
    ...
    const [count] = await db.executeRequest(
      ordersQuery.get("get_orders_count", options) as string
    );
    const pages =
      parseInt(options.$first) > 0
        ? Math.ceil(parseInt(count.COUNT) / parseInt(options.$first))
        : 0;
    return res
      .status(200)
      .json({ count: parseInt(count.COUNT), pages, orders });
  } catch (e) {
    next(e);
  }
};
  ...
  try {
    ...
    for (const status of statuses) {
      if (findStr.includes(status.STATUS_DESCRIPTION.toUpperCase())) {
        ...
        const regX = new RegExp(
          `${status.STATUS_DESCRIPTION.toUpperCase()}`,
          "g"
        );
    ...
    if (queryStatuses.length) {
      queryStatuses.forEach((s) => {
        tempArr.push(`LIST_STATUSES.ID = ${s}`);
      });
    ...
    if (queryKeyword.length) {
      ...
      for (const keywordId of queryKeyword) {
        switch (keywordId) {
          case 1:
            tempArr.push(
              `((O.ORDER_TOTAL_COST - coalesce(O.ORDER_PAY, 0)) * -1) < 0`
            );
            break;
          case 2:
            tempArr.push(`
...
const getSampleForOrder = async (
  ...
  try {
    ...
    let options: QueryOptions = {
      ...ordersQuery.getdefaultOptions("get_order_firstsave_date"),
    };
  ...
  try {
    ...
    if (!result.length)
      return res.status(500).json({
        errors: ["Заказ не найден в базе данных."],
...
const getOneOrder = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Права пользователей.
    const isViewCity: boolean = await user.permissionCompare(
      "Orders [orders] get field City"
    );
    const isViewCost: boolean = await user.permissionCompare(
      "Orders [orders] get field Cost"
    );
    const isViewPay: boolean = await user.permissionCompare(
      "Orders [orders] get field Pay"
    );
    const isViewDebt: boolean = await user.permissionCompare(
      "Orders [orders] get field Debt"
    );

    const isViewDateFirstStage: boolean = await user.permissionCompare(
      "Orders [orders] get field DateFirstStage"
    );
    const isViewDateSave: boolean = await user.permissionCompare(
      "Orders [orders] get field DateSave"
    );
    const isViewDatePlanPack: boolean = await user.permissionCompare(
      "Orders [orders] get field DatePlanPack"
    );
    ...
    const header = resHeader.map((h) => {
      let header: OrderHeader = {
        ...
        manager: h.MANAGER,
        client: h.CLIENT,
        itmOrderNum: h.ITM_ORDERNUM,
        viewMode: h.VIEW_MOD,
        isPrepaid: h.IS_PREPAID,
        profileWidth: h.FASAD_PG_WIDTH,
        assemblyAngle: h.ASSEMBLY_ANGLE,
        prisad: h.PRISAD,
        termoshov: h.TERMOSHOV,
        fasadMaterial: h.FASAD_MAT,
        fasadModel: h.FASAD_MODEL,
        texture: h.TEXTURE,
        textureComment: h.TEXTURE_COMMENT,
        filenkaMaterial: h.FIL_MAT,
        filenkaModel: h.FIL_MODEL,
        filenkaColor: h.FIL_COLOR,
        color: h.COLOR,
        colorType: h.COLOR_TYPE,
        colorLak: h.COLOR_LAK,
        colorLakComment: h.COLOR_LAK_COMMENT,
        colorPatina: h.COLOR_PATINA,
        colorPatinaComment: h.COLOR_PATINA_COMMENT,
        square: h.ORDER_GENERALSQ,
        squareFasad: h.ORDER_FASADSQ,
        comment: h.PRIMECH,
        discount: h.ORDER_DISCOUNT,
        discountComment: h.ORDER_DISCOUNT_COMMENT,
        costPack: h.ORDER_COST_PACK,
        dateLastSave: h.FACT_DATE_LASTSAVE,
        dateCalcCost: h.FACT_DATE_CALCCOST,
        dateFactPack: h.FACT_DATE_PACK,
        dateFactOrderOut: h.FACT_DATE_ORDER_OUT,
        statusNum: h.ORDER_STATUS,
      ...
      if (isViewDebt) header.debt = Math.abs(h.ORDER_DEBT);
...
const getDataHeaderForCreateOrder = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const result = await db.executeRequest<EmployersDb>(
      ordersQuery.get("get_employers")
    );
