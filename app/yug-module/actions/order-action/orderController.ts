...
import { OrderActions, OrderSocketMessage, SocketMessage } from "../../types/socket-message-types";
...
import OrderRoom from "../../engine/OrderRoom";
import Entity from "yug-entity-system-async/dist/Entity";
import { EngineAction, EntityShell } from "yug-entity-system-async/dist/@engine-types";
import YugRoomController from "../../engine/YugRoomController";
import { CreateSampleEntityOrderData, CreateSampleComponentOrderData, AddPropertyToElement, RemovePropertyFromElement, GetAllOrderData, GetSampleNames, EditSampleComponent, EntityPreparationData, GetApiEntityToKey, ChangeEntityComponentToKey, AddOrderElementData, CloseOrderData, CreateOrderData, DeleteOrderElementData, EditOrderElementData, OpenOrderData } from "./interfaces";
 ...
 */
export const notifyOne = async (engine: Engine, subscriber: Subscriber, ...args: any[]) => {
    try {
        const ws = <YugWebsocket>subscriber;
        const service = socketService;
        const [data, message, ...other] = <[data: any, message: OrderSocketMessage, ...other: any[]]>args;
        let msg: OrderSocketMessage | undefined = undefined;

        console.log(subscriber.data, data);
        
        if (data) {
            if ((<EntityShell[]>data)[0]?.options?.key) {
                msg = {
                    method: "order", action: "/get-room-data",
                    data: { order: ((<EntityShell[]>data)?.map(sh => ({ ...sh.options })) || []) },
                    headers: ws.headers
                }
...
export const notifyBroadcast = async (engine: Engine, action: string, ...args: any[]) => {
    const service = socketService;
    const [data, message, entityKey, ...other] = <[data: any, message: SocketMessage, entityKey: string, ...other: any[]] > args;
    const engineAction = <EngineAction> action;
    // Переменные для отправки.
    let msg: OrderSocketMessage | undefined = undefined;
    let condition: (ws: YugWebsocket) => Promise<boolean> = async (ws: YugWebsocket) => false;

    if (engineAction === "create-entity-shell" || engineAction === "update-entity-shell") {
    ...
    if (engineAction === "delete-entity-shell") {
        ...
        msg = {
            method: "order", action: "/delete-room-element",
            data: {
                keys: deletedKeys
            }
        }
 ...
 */
const getRoomClass = (key: string, engine: Engine, ent: Entity): OrderRoom => new OrderRoom(key, engine, ent)

export const createSampleEntity = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<CreateSampleEntityOrderData>>) => {
    try {
        const {  entityDto, components = [] } = msg.data;
        //console.log("entityDto", entityDto, "components", components);
        if (!entityDto || !entityDto.name) throw new Error("Некорректный объект dto");

        const candidate = (await (<YugRoomController>engine.roomController).getEntitySamples()).findIndex(e => e.name === entityDto.name);
        if (candidate > -1) throw new Error("Шаблон сущности с таким именем, уже существует.")

        const entity = await engine.creator.create("entity", entityDto, ...components);

        if (!entity) throw new Error(`Ошибка при создании сущности - шаблона.`)
        openOrder({ ws, service, msg: { ...msg, data: { ...msg.data, entityKey: entity.key, newOrder: true } } }); // Открываем комнату
    } catch (e) {
...
export const createSampleComponent = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<CreateSampleComponentOrderData>>) => {
    try {
        const { componentDto, components = [] } = msg.data;
        if (!componentDto || !componentDto.componentName) throw new Error("Некорректный объект dto");
        const component = await engine.createSampleComponent(componentDto, ...components);

        const cmps = await (<YugRoomController>engine.roomController).getComponentSamples();
 ...
 * @param param0 
 */
export const openOrder = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<OpenOrderData>>): Promise<OrderRoom | null | undefined> => {
    try {
        return new Promise((resolve, reject) => {
            const { entityKey, newOrder } = msg?.data;
            engine.roomController.openRoom(entityKey, (error, room) => {
                if (error || !room) {
                    service.sendError(ws, error || new Error("Непредвиденная ошибка открытия комнаты"));
                    return;
                }
                if (!room) {
                    resolve(null);
                    ...
                    resolve(<OrderRoom>room)
                }
            }, getRoomClass) // Указываем класс своей комнаты (для заказов).
        })
...
export const closeOrder = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<CloseOrderData>>) => {
    try {
        ...
        const room = <OrderRoom | null>engine.roomController.getRoomToKey(roomKey);
        if (room) {
            room.unsubscribe(ws);
        }
...
export const addOrderElement = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<AddOrderElementData>>) => {
    try {
        ...
        engine.roomController.openRoom(roomKey, (error, room) => {
            if (error || !room) {
                service.sendError(ws, error || new Error("Непредвиденная ошибка открытия комнаты"));
                return;
            }
            room.addEntityByKey(entityKey, addedKey)
        }, getRoomClass)
...
export const addPropertyToElement = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<AddPropertyToElement>>) => {
    try {
        ...
        console.log(roomKey, entityKey, propertyKeys);
        engine.roomController.openRoom(roomKey, (error, room) => {
            if (error || !room) {
                service.sendError(ws, error || new Error("Непредвиденная ошибка открытия комнаты"));
                return;
            }
            room.addPropertyToKey(entityKey, propertyKeys)
        }, getRoomClass)
    } catch (e) {
        console.log('\x1b[41m%s\x1b[0m', (e as Error).message);
        service.sendError(ws, e);
...
export const removePropertyFromElement = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<RemovePropertyFromElement>>) => {
    try {
        const { roomKey, entityKey, propertyKey } = msg.data;
        engine.roomController.openRoom(roomKey, (error, room) => {
        ...
        // } else {
        //     ws.rooms = [...ws.rooms.filter(r => r !== entityKey)];
...
export const deleteOrderElement = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<DeleteOrderElementData>>) => {
    try {
        const { roomKey, deletedKey } = msg.data;
        engine.roomController.openRoom(roomKey, (error, room) => {
            if (error || !room) {
                service.sendError(ws, error || new Error("Непредвиденная ошибка открытия комнаты"));
                return;
            }
            room.deleteEntityByKey( deletedKey);
        }, getRoomClass)
...
export const editOrderElement = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<EditOrderElementData>>) => {
    try {
        ...
        engine.roomController.openRoom(roomKey, (error, room) => {
            ...
            room.editEntityToDto(editedKey, propertyKey, dto, ws, service);
        }, getRoomClass)
    } catch (e) {
        console.log('\x1b[41m%s\x1b[0m', (e as Error).message);
        service.sendError(ws, e);
...
export const getAllOrders = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<GetAllOrderData>>) => {
    try {
        ...
        console.log(filter);
        const orders = await engine.events.loadEmit("entity", "Find List", { ...filter, sample: false})
 ...
 * @param param0 
 */
export const getSampleComponents = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<GetSampleNames>>) => {
    const components = await (<YugRoomController> engine.roomController).getComponentSamples();
    service.sender<OrderSocketMessage<{ components: ApiComponent[] }>>(ws, {
        method: 'order',
        action: '/sample-components',
        headers: msg.headers,
 ...
 * @param param0 
 */
export const editSampleComponents = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<EditSampleComponent>>) => {
  try {
    const { componentKey, componentDto } = msg.data;
      ...
      const component = await engine.creator.openSampleComponent(cmp.componentName);
      ...
      if (componentDto.componentDescription  && componentDto.componentDescription !== component.description) {
      ...
      engine.updateComponentApi(...changed).then(cmps => {
        // Обновление в движке
 ...
 * @param param0 
 */
export const getPreparationData = async ({ ws, service, msg }: YugWebsocketAction<OrderSocketMessage<EntityPreparationData>>) => {
    try {
        const { roomKey, entityKey, componentKey } = msg.data;
    
        engine.roomController.openRoom(roomKey, async (error, room) => {
            if (error || !room) {
                service.sendError(ws, error || new Error("Непредвиденная ошибка открытия комнаты"));
                return;
            }
            const entity = await room.getEntityRoom(entityKey);
