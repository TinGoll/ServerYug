...
const data = [
    ...
    },
    {
        ...
        query: (opt) => {
            ...
            let q = `select first 1 
                ...
                O.TEXTURE_COMMENT, O.COLOR_LAK_COMMENT, O.COLOR_PATINA_COMMENT, O.PRISAD, O.PLAN_DATE_FIRSTSTAGE,
                O.PLAN_DATE_PACK, O.FILEPATH_CALC_CLIENT, O.FILEPATH_CALC_MANAGER, O.FILEPATH_BILL, O.VIEW_MOD,
                O.TERMOSHOV, O.ASSEMBLY_ANGLE, LIST_STATUSES.STATUS_NUM,
                coalesce(GET_STATUS_NAME(GET_JSTATUS_ID(O.ID)), LIST_STATUSES.STATUS_DESCRIPTION) as STATUS_DESCRIPTION,
                C.IS_PREPAID, C.CLIENTNAME, C.CITY, C.PRICE_COLUMN, C.IS_PREPAID
                    from ORDERS O
                    left join CLIENTS C on (O.CLIENT = C.CLIENTNAME)
                    left join LIST_STATUSES on (LIST_STATUSES.STATUS_NUM = O.ORDER_STATUS)
    ...
    },
    {
        ...
        query: (opt) => {
            ...
            const { $first = '', $skip = '', $where = '', $sort = '' } = opt;
            let q = `select  E.ID, E.ORDER_ID, E.NAME, E.HEIGHT, E.WIDTH, 
    ...
    },
    {
        ...
        query: (opt) => {
            const { $first = '', $skip = '', $where = '', $sort = '' } = opt;
            let q = `select FACT_DATE_FIRSTSAVE from orders ${$where}`;
            return q;
        },
        defaultOptions: {}
    },
    {
        ...
        query: (opt) => {
            const { $first = '', $skip = '', $where = '', $sort = '' } = opt;
            let q = `select DISTINCT clientname from clients`;
            return q;
        },
        defaultOptions: {}
    },
    {
        ...
        query: (opt) => {
            const { $first = '', $skip = '', $where = '', $sort = '' } = opt;
            let q = `select DISTINCT N.NOMENCLATURE,
                    (select count(L.ID)
    ...
    },
    {
        ...
        query: (opt) => {
            const { $first = '', $skip = '', $where = '', $sort = '' } = opt;
            let q = `select DISTINCT e.name from employers e ${$where}`;
            return q;
        },
